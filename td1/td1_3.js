...
function changeFirstName34 (firstName, separator) {
  // Dans les commentaires suivants, le séparateur est "-"
  // "JEAn-PaTRick" devient ["JEAn", "PaTRick"]
  let arr = firstName.split(separator);
  let arr2 = [];
  // Chaque prénom de arr est transformé et ajouté dans arr2
  arr.forEach(value => arr2.push(value.substring(0, 1).toUpperCase() + value.substring(1)));
  // Chaque élément de arr2 est concanténé dans une chaîne, en ajotant le '-'
...
function encryptString35 (str) {
  ...
  const letters = ['A', 'E', 'G', 'I', 'O', 'S', 'Z'];
  const numbers = [4, 3, 6, 1, 0, 5, 2];
