...
function getNumberOfEven42Alt4(arr) {
  ...
  // on incrémente l'accumulateur à chaque pair
  return arr.reduce((nb, n) => (n % 2 === 0 ? ++nb : nb), 0);
}
...
function sort43(arr1, arr2) {
  ...
  let resultArray = [];
  while (iR < s1 + s2) {
    // aucun tableau n'a encore été transféré
    if (i1 < s1 && i2 < s2) {
      if (arr1[i1] <= arr2[i2]) {
...
function binarySearch44(arr, element) {
  let result = -1;
  let start = 0;
...
console.log(binarySearch44([1, 10, 2, 15, 20, 78], 2));
console.log(binarySearch44([1, 2, 10, 15, 20, 78], 10));
...
console.log(getMaxEven45Alt3(1, 2, 10, 20, 40, 87));
console.log("Quand il n'y a pas de pair, on obtient Number.MIN_SAFE_INTEGER ou -Infinity suivant l'algo utilisé");
console.log(getMaxEven45(1, 87));
console.log(getMaxEven45Alt1(1, 87));
console.log(getMaxEven45Alt2(1, 87));
console.log(getMaxEven45Alt3(1, 87));
